<#
.SYNOPSIS
    PHOTO AI FACTORY — Real Native Installer / Lifecycle Integration Test Harness
#>
...
Write-Host " PHOTO AI FACTORY -- REAL STANDALONE INSTALLER LIFECYCLE TEST" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
...
try {
    ...
    if (-not (Test-Path $installerExe)) {
        $installerExe = "$RepoRoot\src\csharp\PhotoAIFactory.Installer\bin\Release\net10.0-windows10.0.19041.0\PhotoAIFactory-1.0.0-rc.1-Setup.exe"
    }

    # 1. Execute Real Native Setup Executable Standalone
    Write-Host "[1/6] Executing PhotoAIFactory-1.0.0-rc.1-Setup.exe standalone..." -ForegroundColor Yellow
    & $installerExe --install --target-dir "$appInstallDir" --quiet
    if ($LASTEXITCODE -ne 0) {
        throw "Native installer failed with exit code: $LASTEXITCODE"
    }

    if (-not (Test-Path "$appInstallDir\PhotoAIFactory.App.exe")) {
        throw "INSTALLATION FAILED: PhotoAIFactory.App.exe not found in install target."
    }

    # 2. Verify Windows Registry Uninstall Entry
    Write-Host "[2/6] Verifying Windows Uninstall Registry entry..." -ForegroundColor Yellow
    ...
    # 3. First-Run Provisioning Storage Initialization
    Write-Host "[3/6] Initializing per-user application storage..." -ForegroundColor Yellow
    New-Item -ItemType Directory -Force -Path $projectsDir | Out-Null
    New-Item -ItemType Directory -Force -Path $componentsDir | Out-Null
    New-Item -ItemType Directory -Force -Path $modelsDir | Out-Null

    # 4. Simulate Project Lifecycle with Genuine Binary Fixtures
    Write-Host "[4/6] Executing project data lifecycle with genuine SQLite & image fixtures..." -ForegroundColor Yellow
    ...
    # 5. Repair & Upgrade Verification via Standalone Installer
    Write-Host "[5/6] Testing installer repair and upgrade operations..." -ForegroundColor Yellow
    ...
    # 6. Clean Uninstallation Verification via Standalone Installer
    Write-Host "[6/6] Executing real uninstallation and verifying data protection..." -ForegroundColor Yellow
    ...
    Write-Host " REAL STANDALONE INSTALLER LIFECYCLE TESTS: 100% SUCCESS" -ForegroundColor Green
    Write-Host "============================================================" -ForegroundColor Green
}
finally {
    if (Test-Path $TargetDir) {

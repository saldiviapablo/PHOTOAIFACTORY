...
public sealed class MigrationIntegrityException(string message)
    : IOException(message);
...
public static class MigrationCatalog
{
    ...
    [
        ...
        new(7, "comfyui", ReadEmbedded("007_comfyui.sql")),
        new(8, "qa_review_publish", ReadEmbedded("008_qa_review_publish.sql"))
    ];
    ...
    IReadOnlyList<SqliteMigration> migrations)
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var existedWithContent =
            File.Exists(database.DatabasePath) &&
            new FileInfo(database.DatabasePath).Length > 0;

        await using var writerLease =
            await database.Writer.EnterAsync(cancellationToken)
                .ConfigureAwait(false);
        await using var connection =
            database.CreateConnection(createIfMissing: true);
        ...
        if (pending.Length == 0)
        {
            await SqliteProjectDatabase.ConfigureConnectionAsync(
                connection, cancellationToken).ConfigureAwait(false);
        ...
        {
            LastBackupPath = await BackupAsync(
                connection, cancellationToken).ConfigureAwait(false);
        }
            ...
            foreach (var migration in pending)
            {
                ...
                record.CommandText = """
                    INSERT INTO schema_migrations(
                        version, name, migration_sha256, applied_at_utc)
                    VALUES(
                        $version, $name, $sha256, $appliedAtUtc);
                    """;
    ...
    private void ValidateMigrationList()
    {
        ...
        {
            throw new InvalidOperationException(
                "At least one valid migration is required.");
        }
        ...
        if (!ordered.SequenceEqual(
                migrations.Select(item => item.Version)) ||
            ordered.Distinct().Count() != ordered.Length)
        {
            throw new InvalidOperationException(
                "Migrations must be uniquely ordered by version.");
        }
    }

    private void ValidateApplied(
        IReadOnlyDictionary<int, AppliedMigration> applied)
    {
        foreach (var item in applied)
        {
            var known = migrations.SingleOrDefault(
                migration => migration.Version == item.Key)
                ?? throw new MigrationIntegrityException(
                    $"Database contains unknown migration {item.Key}.");
            ...
            {
                throw new MigrationIntegrityException(
                    $"Applied migration {item.Key} name or SHA-256 " +
                    "differs from the executable catalog.");
            }
        }
    }

    private static async Task<Dictionary<int, AppliedMigration>>
        ReadAppliedAsync(
            ...
            CancellationToken cancellationToken)
    {
        ...
        command.CommandText = """
            SELECT version, name, migration_sha256
    ...
    private static async Task EnsureWalAsync(
        SqliteConnection connection,
        CancellationToken cancellationToken)
    {
        await using var command = connection.CreateCommand();
        ...
        {
            throw new InvalidOperationException(
                $"SQLite refused WAL mode; reported '{value}'.");
        }
    }
